...
class FileEncodingTest {
    ...
    @Test fun file_mkdir_creates_dir() {
        setupActivity()
        val result = mruby.eval("Mrboto::Helpers.file_mkdir('/data/local/tmp/mrboto_test_dir').to_s")
        assertEquals("true", result)
    }

    @Test fun file_delete_dir_removes_dir() {
        setupActivity()
        val result = mruby.eval("Mrboto::Helpers.file_delete_dir('/data/local/tmp/mrboto_test_dir').to_s")
        assertEquals("true", result)
    }
