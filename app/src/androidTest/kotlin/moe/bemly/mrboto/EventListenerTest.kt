...
class EventListenerTest {
    ...
    @Test fun event_listener_methods_exist() {
        assertEquals("true", mruby.eval("Mrboto::Helpers.respond_to?(:observe_battery).to_s"))
        assertEquals("true", mruby.eval("Mrboto::Helpers.respond_to?(:observe_network).to_s"))
        assertEquals("true", mruby.eval("Mrboto::Helpers.respond_to?(:network_connected?).to_s"))
    }

    @Test fun network_connected_returns_boolean() {
        setupActivity()
        val result = mruby.eval("Mrboto::Helpers.network_connected?.to_s")
